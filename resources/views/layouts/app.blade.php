    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
        <form method="POST" action="{{ route('logout') }}" class="form-inline">
          @csrf
          <button type="submit" class="btn btn-link nav-link" style="color: #000;">Logout</button>
        </form>
      </li>
    </ul>
  </nav>
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="{{ route('admin.dashboard') }}?view=users" class="nav-link {{ request('view')=='users' ? 'active' : '' }}">
